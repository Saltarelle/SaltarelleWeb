	...
	public class WebIDLResolver {
		...
		private void Prepare(Definition def) {
			def.Decompose(
				...
				implements => {	// Handled in the next step
				},
				exception => {
		...
		private void BuildResolvedModel() {
			foreach (var e in _entities.Values.ToList()) {	// Necessary to copy collection since Resolve() might declare interfaces for all undeclared symbols
